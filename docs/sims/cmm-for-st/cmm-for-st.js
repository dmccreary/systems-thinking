...
let containerWidth;
const canvasHeight = 550; // Fixed vertical size
let canvasWidth;
...
function setup() {
  ...
  descriptions = [
    ...
    "Level 2 - Aware: Leadership begins recognizing that organizational issues are interconnected. Some consideration of secondary effects in strategic planning, though analysis remains limited. Basic stakeholder mapping occurs, and there's growing awareness that quick fixes often backfire. Decision-making still primarily reactive, but questions about root causes are starting to emerge.",
    ...
    "Level 4 - Integrated: Systems thinking is embedded throughout strategic decision-making processes. Organizations use systems archetypes to understand recurring patterns and identify leverage points for maximum impact. Cross-functional collaboration is standard, with shared systems vocabulary. Decision-making explicitly balances short-term needs with long-term system health and sustainability.",
  ...
  describe(
    'Systems Thinking Strategic Decision Maturity Model showing five levels of organizational maturity from Linear to Transformative, displayed as ascending steps with detailed descriptions available on hover.',
    LABEL
  );
}
...
function draw() {
  ...
  text("Systems Thinking Strategic Decision Maturity", canvasWidth / 2, 10);
  ...
  if (currentHover !== -1) {
    ...
    stroke(150);
    strokeWeight(1);
    rect(10, 450, canvasWidth - 20, 95, 5);
    fill(0);
    noStroke();
    textSize(14);
    textAlign(LEFT, TOP);
